...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    while True:
        try:
            # Increment image index and shuffle if at the start of an epoch
            ...
            if dataset.image_info[image_id]['source'] in no_augmentation_sources:
                image, image_meta, gt_class_ids, gt_boxes, gt_masks = load_image_gt(
                    dataset, config, image_id, augment=augment, augmentation=None, use_mini_mask=config.USE_MINI_MASK)
            else:
                image, image_meta, gt_class_ids, gt_boxes, gt_masks = load_image_gt(
                    dataset, config, image_id, augment=augment, augmentation=augmentation, use_mini_mask=config.USE_MINI_MASK)
            ...
            # Mask R-CNN Targets if random_rois is set
            if random_rois:
            ...
            # Subsample instances if there are more than MAX_GT_INSTANCES
            ...
            # If batch is full, yield the data
            if b >= batch_size:
                ...
                if random_rois:
                    ...
                    if detection_targets:
                        ...
                        # Ensure output and targets have the same dimensions
...
#            self.config.NAME.lower(), now))
    ...
    # Mantén esta lógica de configuración
    	...
    	self.keras_model.fit(
        	...
        	steps_per_epoch=int(tf.data.experimental.cardinality(train_dataset_tf).numpy()) // self.config.BATCH_SIZE,
        	validation_steps=int(tf.data.experimental.cardinality(val_dataset_tf).numpy()) // self.config.BATCH_SIZE,
    	)
